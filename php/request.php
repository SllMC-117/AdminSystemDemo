...
if ($data->accion === "validarLogin") {
    ...
    $query->execute(array($data->user, 1));

    //Encontro coincidencia de credenciales de Usuario
    if ($query->rowCount() > 0) {
        $datosUsuario = $query->fetchAll(); //Guardamos valores de datosUsuario que necesitamos
        // print_r($datosUsuario); exit;
        $idUser = $datosUsuario[0]->rowid;
        $userName = $datosUsuario[0]->name;
        $oldHash = $datosUsuario[0]->token; //Stored hash of Tkn
        ...
        //verify stored hash against plain-text psw
        if (password_verify($psw, $oldHash)) {
            //Verify legacy psw to new password_hash options only if we elevate the Security
            if (true === password_needs_rehash($oldHash, $newHash['algo'], $newHash['options'])) {
                // Rehash/Store plain-text psw using new hash
                $newPswHash = password_hash($psw, $newHash['algo'], $newHash['options']);

                //Store new hash Tkn in DB
                $sql = "
                    UPDATE 
                        tbl_users 
                    SET 
                ...
                //Query exitosa
                if ($querySuccess) {
                    //Regresamos respuesta con datos de Usuario
                    $respuesta->error = false;
                    $respuesta->mensaje = "Bienvenido $userName";
                    $respuesta->confirmBtn = false;
                    $respuesta->userName = $userName;
                } else {
                    echo "Error en ejecución de consulta";
                }
            } else { //Hash still the best security standard
                //Regresamos respuesta con datos de Usuario
                $respuesta->error = false;
                $respuesta->mensaje = "Bienvenido $userName";
                $respuesta->confirmBtn = false;
            ...
            $respuesta->mensaje = "Error, credenciales incorrectas.";
...
if ($data->accion === "leftMenu") {
    $sql = "
        SELECT 
            s.rowid as idSeccion, 
            s.seccion,
            s.nombre,
            s.activo,
            s.orden
    ...
    foreach ($secciones as $key => $row) {
        if ($row->seccion === 'Banners') {
            $row->iconSeccion = 'fa-images';
        } elseif ($row->seccion === 'Reviews') {
            $row->iconSeccion = 'fa-star-half-alt';
        } else {
